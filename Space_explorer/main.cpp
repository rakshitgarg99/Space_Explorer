...
GLint flag = 0, flag1 = 0, flag2 = 0, flag3 = 0, flag5 = 1, flag4 = 0; // for performing actions and calling msgs
GLfloat sp1 = 0.05, sp2 = 0.07, sp3 = 0.0625; // for controlling speed of obstacles(sp1, sp2) and plane(sp3)
void init()
{
	glMatrixMode(GL_PROJECTION); // info we have in buffer we display it in 2d matrix so it take info from buffer or perspective transformation or orthogonal transformation
	glLoadIdentity(); // replaces the current matrix with the identity matrix
	gluOrtho2D(0, 600, 0, 600);
	glMatrixMode(GL_MODELVIEW); // deals with matrices used by model-view transformation
}

void drawstring(float x, float y, float z, char* string)
{
	char* c;
	glRasterPos3f(x, y, z); // used to position pixel and bitmap write operations
	for (c = string; *c != '\0'; c++)
	{
		glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, *c); // renders a bitmap character
	}
}
void screen()
{
	glColor3f(1.0, 0.0, 1.0);
	drawstring(210, 540, 0.0, "CG MINI PROJECT ON-SPACE EXPLORER");
	drawstring(290, 500, 0.0, "BY");
	glColor3f(1.0, 1.0, 0.2);
	drawstring(180, 420, 0.0, "NITIN GUPTA");
	drawstring(340, 420, 0, "101903743");
	drawstring(180, 370, 0.0, "RAKSHIT");
	drawstring(340, 370, 0.0, "102083022");
	glColor3f(1.0, 0.0, 1.0);
	drawstring(230, 320, 0.0, "UNDER THE GUIDENCE OF");
	glColor3f(1.0, 1.0, 0.2);
	drawstring(260, 280, 0.0, "Ms. Rupali Goyal");
	drawstring(160, 240, 0.0, "THAPAR INSTITUTE OF ENGINEERING AND TECHNOLOGY");
	glColor3f(0.0, 0.8, 1.0);
	drawstring(160, 130, 0.0, "DEPARTMENT OF COMPUTER SCIENCE AND ENGINEERING");
	drawstring(230, 80, 0.0, "ACADEMIC YEAR: 2021-22");
...
void display(void)
{
	glClear(GL_COLOR_BUFFER_BIT); // stores the color values in buffer bit which we given in clear color command
	glClearColor(0.0, 0.0, 0.0, 0.0); // background color of the window

	if (flag == 0)
	{
		screen(); // the name, project info window
	}
	if (flag1 == 1) // renders plane and obstacles
	{
		plane();
		obstacle();
	...
	{
		flag1 = 0;
		msg2(); // level 1 completed
	}
	if (flag2 == 1 && sp3 == 0.125)
	{
		flag1 = 0;
		msg3(); // level 2 completed
	}
	if (flag5 == 0)
	{
		flag1 = 0;
		msg(); // congrats msg for next level
		flag3 = 1;
	}
	if (flag4 == 1)
	{
		flag = 1; // reset all the values as all levels are completed
		flag1 = 0;
		flag2 = 0;
		flag5 = 1;
...
{
	glutInit(&argc, argv); // to use glut we have to intialise all its arguments
	glutInitDisplayMode(GLUT_SINGLE | GLUT_RGB); // use single buffer to store intensity values of pixels
	...
	glutIdleFunc(mov); // sets the global idle callback to be func and help in process the glut code in background
	glutKeyboardFunc(key); // get the pressed keyboard as input and perform the task
	glutMainLoop();
	return 0;
}
